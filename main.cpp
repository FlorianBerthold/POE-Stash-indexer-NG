...
void parse_JSON(const std::string path) {
    ...
    sql::PreparedStatement *account_stmt = processing_con->prepareStatement(
            "INSERT INTO `Accounts` (`accountName`, `lastCharacterName`, `lastSeen`) VALUES (?, ?, ?) ON DUPLICATE KEY UPDATE `lastSeen` = ?, `lastCharacterName` = ?");
    sql::PreparedStatement *stash_stmt = processing_con->prepareStatement(
            "INSERT INTO `Stashes` (`stashId`, `stashName`, `stashType`, `publicStash`) VALUES (?, ?, ?, ?) ON DUPLICATE KEY UPDATE `stashName` = ?, `stashType` = ?, `publicStash` = ?");
    sql::PreparedStatement *league_stmt = processing_con->prepareStatement(
            "INSERT INTO `Leagues` (`leagueName`, `active`, `poeTradeId`) VALUES (?, '1', ?) ON DUPLICATE KEY UPDATE `leagueName` = `leagueName`");
    sql::PreparedStatement *item_stmt = processing_con->prepareStatement(
...
void mod_loop() {
    ...
    while (true) {
        ...
        if (!inserting_mods && mod_queue.size() > 0) {
            ...
            threaded_insert("LOAD DATA CONCURRENT INFILE '" + path.str() +
                            "' REPLACE INTO TABLE `Mods` FIELDS TERMINATED BY ',' ENCLOSED BY '\"' ESCAPED BY '\"' LINES TERMINATED BY '\n'");
...
void requirement_loop() {
    ...
    while (true) {
        ...
        if (!inserting_requirements && requirement_queue.size() > 0) {
            ...
            threaded_insert("LOAD DATA CONCURRENT INFILE '" + path.str() +
                            "' REPLACE INTO TABLE `Requirements` FIELDS TERMINATED BY ',' ENCLOSED BY '\"' ESCAPED BY '\"' LINES TERMINATED BY '\n'");
            std::remove(path.str().c_str());
            path.str("");
            std::chrono::steady_clock::time_point end = std::chrono::steady_clock::now();
            time_requirements += (std::chrono::duration_cast<std::chrono::microseconds>(end - begin).count() /
                                  1000000.0);
...
void property_loop() {
    ...
    while (true) {
        ...
        if (!inserting_properties && property_queue.size() > 0) {
            ...
            threaded_insert("LOAD DATA CONCURRENT INFILE '" + path.str() +
                            "' REPLACE INTO TABLE `Properties` FIELDS TERMINATED BY ',' ENCLOSED BY '\"' ESCAPED BY '\"' LINES TERMINATED BY '\n'");
            std::remove(path.str().c_str());
            path.str("");
            std::chrono::steady_clock::time_point end = std::chrono::steady_clock::now();
...
void socket_loop() {
    ...
    while (true) {
        ...
        if (!inserting_sockets && socket_queue.size() > 0) {
            ...
            threaded_insert("LOAD DATA CONCURRENT INFILE '" + path.str() +
                            "' REPLACE INTO TABLE `Sockets` FIELDS TERMINATED BY ',' ENCLOSED BY '\"' ESCAPED BY '\"' LINES TERMINATED BY '\n'");
            std::remove(path.str().c_str());
            path.str("");
            std::chrono::steady_clock::time_point end = std::chrono::steady_clock::now();
